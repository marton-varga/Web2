...
<div class="loginbox" id="ujfilmbox">
	...
	<form action="" method="post" novalidate>
		...
		<select onchange="ujfilm()" id="filmlista" name="cim">
			<?php
				foreach($filmek as $f){
					echo "<option value='" . $f->cim . "'";
					if(isset($_POST['cim']) && $_POST['cim']==$f->cim){echo " selected";}
					elseif(!isset($_POST['cim']) && $f->cim == $vetites->cim){echo " selected";}
					echo ">";
					echo $f->cim;
					echo "</option>";
				}
		...
		<select name="terem">
			<?php
				...
				for($i=0; $i<count($termek);$i++){
					...
					if(isset($_POST['terem']) && $_POST['terem']==$termek[$i]){echo " selected";}
					elseif(!isset($_POST['terem']) && $vetites->terem==$termek[$i]){echo " selected";}
					echo ">";
					echo $termek[$i];
					echo "</option>";
				}
		...
		<input type="number" name="helyek" min="1" value="<?php
			if(isset($_POST['helyek'])){echo $_POST['helyek'];}
			else{echo $vetites->helyek;}
			?>">
		<br>
			<?php hibakiir('helyek'); ?>
		<label for="ar">Ár:</label>
		<input type="number" name="ar" min="1" value="<?php
			if(isset($_POST['ar'])){echo $_POST['ar'];}
			else{echo $vetites->ar;}
			?>">
		<br>
			<?php hibakiir('ar'); ?>
		<label for="aktiv">Aktív:</label>
		<input type="checkbox" name="aktiv" value="aktiv" <?php
			if(isset($_POST['aktiv'])){echo "checked";}
			elseif($vetites->aktiv){echo "checked";};
			?>
		>
		<br>
	...
	<form action="torlesjovahagyas.php?vetites=<?= $vetites->id ?>" method="post">
	<input type="hidden" name="del" value="del">
	<input type="submit" value="Vetítés törlése">
	</form>
</div>
</main>
<script>
